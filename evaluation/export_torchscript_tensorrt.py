...
f_post = torch.jit.load(out_base_dir / f"{model_prefix}_float32_jit_cuda_post.ts")
...
def model_trt(data, f_enc, f_msg, f_post, f_bev, f_bevdec, dtype=torch.float):
    ...
    f_post_trt = torch_tensorrt.compile(
        ...
        inputs=[
            torch_tensorrt.Input(
                ...
                opt_shape=[8, 17],
                max_shape=[360, 17],
                dtype=torch.float32,      # FP16 I/O
            )
        ],
        enabled_precisions={torch.float32},  # FP16 kernels
        truncate_long_and_double=True,
    )
    ...
    # return f_enc_trt, f_msg_trt, None, None, None
