...
class BEVGNNModel(nn.Module):
    ...
    def forward(self, input: Dict[str, Tensor]) -> Tuple[Tuple[Dict[str, Tensor], Tensor], Tuple[Tensor, Tensor]]:
        ...
        graphs.x = x[:, : self.gnn_in_seq_len] # encoder done
        ...
        z_p = self.pose_gnn(x=graphs.x, edge_index=graphs.edge_index)
        ...
        z = self.bev_gnn(x=graphs.x, edge_index=edge_index_self, edge_attr=edge_attr_self)
        ...
        top  = int(H / 2 - 30)
        left = int(W / 2 - 30)
        result_crop = dec[..., top:top+60, left:left+60]
